"use client";

import React, { useState } from 'react';
import { X, Check } from 'lucide-react';

const activities = [
  "Daily Cardiovascular Exercise (30 mins)",
  "Increased Fiber Intake",
  "Hydration Goal: 3 Liters/Day",
  "Consistent Sleep Schedule",
  "Stress Management / Meditation",
  "Routine Blood Pressure Check"
];

export default function HealthcareRec() {
  const [selected, setSelected] = useState<string[]>([]);

  const toggleActivity = (activity: string) => {
    setSelected(prev => 
      prev.includes(activity) 
        ? prev.filter(item => item !== activity) 
        : [...prev, activity]
    );
  };

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm">
    <div className="max-w-3xl mx-auto p-10 bg-white w-[100vh] rounded-xl shadow-sm border border-gray-100">
      {/* Header */}
      <div className="flex justify-between items-center mb-2">
        <h2 className="text-2xl font-bold text-slate-800">Recommend List</h2>
        <button className="flex items-center text-gray-500 hover:text-gray-800 transition-colors">
          <span className="mr-1 text-sm">close</span>
          <X size={18} />
        </button>
      </div>
      
      <p className="text-right text-slate-500 italic text-sm mb-6">
        Click square to select activity
      </p>

      {/* List Items */}
      <div className="space-y-3">
        {activities.map((activity) => {
          const isSelected = selected.includes(activity);
          return (
            <div 
              key={activity}
              onClick={() => toggleActivity(activity)}
              className="group flex items-center justify-between p-4 bg-gray-100 rounded-xl cursor-pointer hover:bg-gray-200 transition-all"
            >
              <span className="text-slate-600 font-medium">{activity}</span>
              
              <div className={`
                w-7 h-7 rounded-lg border-2 flex items-center justify-center transition-colors
                ${isSelected 
                  ? 'bg-blue-500 border-blue-500' 
                  : 'bg-white border-gray-300 group-hover:border-gray-400'}
              `}>
                {isSelected && <Check size={18} className="text-white" />}
              </div>
            </div>
          );
        })}
      </div>

      {/* Footer Action */}
      <div className="mt-8 flex justify-end">
        <button 
          disabled={selected.length === 0}
          className={`
            px-6 py-3 rounded-xl font-medium transition-all
            ${selected.length > 0 
              ? 'bg-blue-50 text-blue-600 hover:bg-blue-100' 
              : 'bg-gray-50 text-gray-300 cursor-not-allowed'}
          `}
        >
          Add to Health care activity
        </button>
      </div>
    </div>
</div>
  );
}